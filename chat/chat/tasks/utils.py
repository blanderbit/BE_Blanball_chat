...
def find_user_in_chat_by_id(*, users: list[int], user_id: int) -> Optional[dict[str, Any]]:
...
def check_user_is_chat_member_and_not_author(*, chat: Chat, user_id: int) -> bool:
    return any(user.get("user_id") == user_id and not user.get("author") for user in chat.users)


def check_user_is_chat_author(*, chat: Chat, user_id: int) -> bool:
    return any(user.get("user_id") == user_id and user.get("author") for user in chat.users)
